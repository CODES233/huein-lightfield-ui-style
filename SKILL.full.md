...
## 十四、宽屏：别把 2560 当 1440 用

内容区如果死守一个宽度（比如 1320px）且靠左，在 2560 的屏上右侧会空出近 1000px ——
**等于把大屏当小屏用**。这套风格的做法是**三档媒体查询**，同时放宽内容区和侧栏：

| 视口 ≥ | 内容区 | 侧栏 |
|---|---|---|
| 1680 | 1540px | 244px（默认） |
| 2000 | 1780px | 268px |
| 2400 | 2060px | 288px |

- 栅格一律用 `auto-fit + minmax` —— 内容区一放宽，卡片列数自动变多，不用另写规则
- ⚠️ 别把侧栏宽度硬编码进断言 —— 宽屏下它是 268 / 288，不是 244。**动态量**
- 浅色主题下卡片玻璃 `--glass-bg` 要比暗色更实（0.86 vs 0.55），
  不然 92% 那种不透明度会把身后的光场完全挡掉
...
**不要只取内容区的直接子元素** —— 那些大多是布局壳（`div.grid` 里装着 4 张卡、
`div.split` 里装着两栏、`div.kanban` 里装着 7 列）。给壳做动画的结果是整页一起淡入。

### 判据：一直往下走到「整块」为止

```
对内容区的每个顶层块：
  · 它自己标了 data-reveal   → 就是它（unfold 例外，见 ⑤）
  · 里面有人标了 data-reveal → 拆开取里面那些，**别把外层壳也算一块**
  · TABLE / DETAILS          → 整块（理由见 ③）
  · .card / FORM             → 整块 —— 除非它是「一排块的壳」（见 ②）
  · 同层有「≥3 个同类兄弟」   → 那一组各自是块
  · 其余（div / section / …）→ 继续往下拆
  · 拆不动                    → 就它自己
```

### ⚠️ 七条踩出来的硬规则

**① 每一层的并列组要「全收」，不能只取最大的一组。**

这是这套风格里最贵的一个 bug。原来的实现每层只返回**最大的那一组**，
于是同一层里其它并列块**永远拿不到动画**。实测量到的后果：

| 页面 | 只选中了 | 结果 |
|---|---|---|
| `/series` 内容线管理 | 3 个 `div.field` | 左栏 + 右边两张卡 **0/4 没有动画** |
| `/roles` 角色与权限 | 左栏那 7 个导航项 | 右侧 982×2636 的整个面板 **0/6** |
| `/audit` 操作审计 | 12 个表格行 | 两张卡片自己 **0/2** |
| `/ai` AI 助手 | 14 个表格行 | 三张卡片 **4/6** |

用户的原话是「以下页面都有**部分控件缺失载入动画**」，点名的正是这些页面。

> **加一条断言防它回来**：内容区里每一个 `.card` 都必须「自己或某个祖先」是浮入单元。
> 这条断言能直接量出上面四种漏法，而且不看代码就能跑。

**② `.card` 与 `FORM` 是内容单元，不要往里钻。**

往里钻的后果实测过：**卡片框先出现、里面的表格行再一行行浮起来** ——
读起来正是「这张卡片缺了载入动画」。同类问题早先在 `/ideas` 上也踩过一次
（折叠卡里的 3 个表单字段被当成单元，卡片自己反而没有动画）。

**例外只有一条**：它是「一排块的壳」—— 里面装着 ≥3 张同类卡片。
`/today` 的「快捷入口」（5 张链接卡）、`/about` 的「权限点分布」（4 张统计卡）、
`/settings` 那个 1335px 的表单（4 张设置卡）都是这一类，要往下拆。

⚠️ 判「是壳」要**往下多看两层** —— `card > div.grid > a.card ×5` 才是真实结构，
只看直接孩子会漏掉。壳卡片自己不动（和 `div.grid` 一样）是刻意的：
它的每一部分都在动，读起来不缺任何东西。

**③ `TABLE` 与 `DETAILS` 永远是一整块。**

- **表格**：表头 `thead` 和数据行 `tbody` 是一体的。只让行浮入、表头纹丝不动，
  看起来像坏了；而且 200 行的表会把单元数打爆（见 ④）。
  → 顺手把 `TR` 也放进「永不当组员」的名单里，双保险。
- **`<details>`**：内容收起来就是隐藏的。往里拆会把动画
  **从看得见的东西偷到看不见的东西上**。

**④ 上限不能是「截断」。**

`return units.slice(0, 16)` 会**静默丢掉尾巴**：末尾那几张卡永远不动，看起来像坏了。
改成「拆出来会超上限就整块进来」：

```js
if (parts && parts.length && units.length + parts.length <= REVEAL_MAX_UNITS) {
  units.push(...parts);
  return;
}

units.push(el);          // 整块进来，绝不丢
```
...
壳和里面的块同时播 = 两层动画叠加：位移 16+16px、模糊 8+8px。
看起来像「这一块比别人糊一倍」，而不是「这块更重」。

**⑥ 判「可见」要算上祖先裁剪。**

折叠面板（`.collapse` 用 `grid-template-rows` + `overflow: hidden`）里的东西，
`getBoundingClientRect()` 照样给你一个 712px 高的框 —— **裁剪不改变元素的盒子**。
不按这个判，`/team` 里「6 个折叠着的成员面板」会被当成「一排并列的卡片」拆出来，
动画全落在看不见的地方。
...
这类元素可能永远达不到 IO 的门槛。**把它从编排里排除，等于让它不依赖 JS 和动画机制，
永远可见。**
